...
public class HerosModel extends AbstractMedlabsModel {
			...
			for (Location location : locationType.getLocationMap().valueCollection()) {
				if (location.getId() >= 0) {
					this.locationNrWriter
							.print(time + "," + location.getId() + "," + location.getAllPersonIds().size() + "\n");
					for (int subLocationIndex = 1; subLocationIndex < location
							.getNumberOfSubLocations(); subLocationIndex++) {
						int nrSub = this.diseaseTransmission.getNrPersonsInSublocation(location,
								(short) subLocationIndex);
	...
	protected void checkChangeWeekPattern() {
		// System.out.println("checkChangeWeekPattern @ " +
		// getSimulator().getSimulatorTime() + " h");
		for (Person person : getPersonMap().valueCollection()) {
			...
			if (!getWeekPatternMap().containsKey(newWeekPatternName)) {
				...
				// throw new MedlabsRuntimeException("Week pattern " + newWeekPatternName + "
				// not found");
			}
	...
	protected void extendInputParameterMap() throws InputParameterException {
		...
		genericMap.add(
				new InputParameterString("ProbRatioFilePath", "path and name for the probability based infections file",
		...
		genericMap
				.add(new InputParameterString("diseasePropertiesFile", "path and name for the disease properties file",
		...
		policyMap.add(
				new InputParameterInteger("NumberInfected", "number of people infected at t=0", "(can be 0)", 0, 1.0));
		policyMap.add(new InputParameterInteger("MinAgeInfected", "lowest age of people infected at t=0",
				"between 0 and 100.", 0, 0, 100, "%d", 2.0));
		policyMap.add(new InputParameterInteger("MaxAgeInfected", "highest age of people infected at t=0",
				"between 0 and 100.", 100, 0, 100, "%d", 3.0));
		...
		InputParameterMap covidTransmissionDistMap = new InputParameterMap("covidT_dist",
				"Covid Transmission by Distance", "Covid Transmission parameters by distance", 1.51);
		covidTransmissionDistMap.add(new InputParameterDouble("L", "Latent period L (days)", "Viral load model", 2.0,
				0.0, 100.0, true, true, "%f", 1.0));
		...
		covidTransmissionDistMap.add(new InputParameterDouble("v_max", "Peak viral load v_max", "Viral load model; > 0",
				7.23, 0.0, 100.0, true, true, "%f", 4.0));
		...
		covidTransmissionDistMap.add(new InputParameterDouble("r", "Transmission rate r",
				"Transmission probability; > 0", 2.294, 0.0, 100.0, true, true, "%f", 6.0));
		...
		covidTransmissionDistMap.add(new InputParameterDouble("alpha", "Calibraton factor alpha",
				"Infection probability; > 0", 5.0, 0.0, 100.0, true, true, "%f", 8.0));
		...
		InputParameterMap covidProgressionMap = new InputParameterMap("covidP", "Covid Progression",
				"Covid Progression parameters", 1.6);
		covidProgressionMap
				.add(new InputParameterDouble("FractionAsymptomatic", "Fraction that is asymptomatic E->I(A)",
						"Probability between 0.0 and 1.0", 0.46, 0.0, 1.0, true, true, "%f", 1.0));
		covidProgressionMap
				.add(new InputParameterString("IncubationPeriodAsymptomatic", "Incubation period asymptomatic E->I(A)",
						"Distribution, time in days", "Triangular(2.5, 3.4, 3.8)", 2.0));
		covidProgressionMap
				.add(new InputParameterString("IncubationPeriodSymptomatic", "Incubation period symptomatic E->I(S)",
						"Distribution, time in days", "Triangular(2.5, 3.4, 3.8)", 3.0));
		covidProgressionMap.add(
				new InputParameterString("PeriodAsymptomaticToRecovered", "Period asymptomatic to recovered I(A)->R",
						"Distribution, time in days", "Triangular(7, 12, 14)", 4.0));
		...
		covidProgressionMap.add(new InputParameterString("PeriodSymptomaticToHospitalized",
				"Period symptomatic to hospitalized I(S)->I(H)", "Distribution, time in days", "Triangular(5, 7.5, 10)",
				6.0));
		covidProgressionMap
				.add(new InputParameterString("PeriodSymptomaticToRecovered", "Period symptomatic to recovered I(S)->R",
						"Distribution, time in days", "Triangular(5, 7.5, 10)", 7.0));
		covidProgressionMap.add(new InputParameterString("FractionHospitalizedToICU",
				"Fraction hospitalized to ICU I(H)->I(I)", "Fraction, age{} map or gender{} map",
				"age{0-29: 0.05, 30-49: 0.02, 50-59: 0.05, 60-80: 0.15, 80-100: 0.0}", 8.0));
		covidProgressionMap.add(new InputParameterString("FractionHospitalizedToDead",
				"Fraction hospitalized to dead I(H)->D", "Fraction, age{} map or gender{} map", "0.0", 9.0));
		...
		covidProgressionMap.add(new InputParameterString("FractionICUToDead", "Fraction ICU to dead I(I)->D",
				"Fraction, age{} map or gender{} map",
				"age{0-49: 0, 50-59: 0.015, 60-69: 0.08, 70-79: 0.4, 80-100: 0.6}", 13.0));
	...
	public void checkChangeActivityPattern(final Person person) {
		// System.out.println("checkChangeActivityPattern @ " +
		// getSimulator().getSimulatorTime() + " h. for person " + person);
		if (person.getDiseasePhase().isDead())
			return;
		...
		if (getWeekPatternMap().get(weekPatternKey) == null) {
			System.err.println("checkChangeActivityPattern - Week pattern key" + weekPatternKey
					+ " not found in week pattern map");

...
public class ConstructHerosModel {
	...
	public ConstructHerosModel(final HerosModel model) {
		...
		try {
			...
			DiseaseTransmission covidTransmission = (this.model.getParameterValue("generic.diseasePropertiesModel")
					.equals("area")) ? new Covid19TransmissionArea(this.model)
							: new Covid19TransmissionDistance(this.model);
	...
	private File getPathFromParam(final String param, final boolean dir) {
		...
		if (file.isDirectory() && !dir) {
			System.err.println("parameter " + param + " with value: " + paramValue
					+ " should point to a file, but it is a directory");
			System.exit(-1);
		}
		if (!file.isDirectory() && dir) {
			System.err.println("parameter " + param + " with value: " + paramValue
					+ " should point to a directory, but it is a file");
			System.exit(-1);
		}
		return file;
	}
	...
	private File getFileFromParam(final String param, final String defaultFileName) {
		...
		throw new RuntimeException(
				"could not find path as specified in parameter " + param + " with value: " + paramValue);
	}

	@SuppressWarnings("unchecked")
	private void readLocationTypeTable() throws Exception {
		byte id = 0;
		// House or Accommodation should be #0, the first in the list (see HerosModel,
		// method getLocationTypeHouse.
		...
		if (it.hasNext()) {
			...
			while (it.hasNext()) {
				...
				Class<? extends LocationAnimation> ltAniClass = (Class<? extends LocationAnimation>) Class
						.forName(ltAniClassName);
				...
				if (locationType == null) {
					for (byte b = (byte) 0; b < 128; b++) {
						if (!this.model.getLocationTypeIndexMap().containsKey(b)) {
							...
							locationType = new LocationType(this.model, b, locationCategory, Location.class, null,
									false, true, 1.0);
							break;
						}
					}
				...
				if (diseasePhase.getName() != "Dead") {
					for (String personType : new String[] { "infant", "kindergarten student", "primary school student",
						...
						if (this.model.getWeekPatternMap()
							...
							System.err.println("Week pattern " + policy + "_" + diseasePhase.getName() + "_"
	...
	private void makePersonTypes() {
		int nr = 1;
		for (Class<? extends Person> pc : new Class[] { Infant.class, KindergartenStudent.class,
				PrimarySchoolStudent.class, SecondarySchoolStudent.class, CollegeStudent.class, UniversityStudent.class,
				Worker.class, Pensioner.class, Unemployed.class, WeekendWorker.class, EssentialWorker.class,
				WorkerSatelliteToCity.class, WorkerCityToSatellite.class, WorkerSatelliteToSatellite.class,
				WorkerCountryToCity.class }) {
	...
	private void readPersonTable() throws Exception {
		...
		if (it.hasNext()) {
			...
			while (it.hasNext()) {
				...
				if (age < 0 || age > 120) {
					System.err.println("Person " + personId + " has age " + age + " on row "
				...
				// if (this.model.getLocationMap().get(homeId).getLocationTypeId() !=
				// this.model.getLocationTypeHouse()
				// .getLocationTypeId())
				// {
				// System.err.println("homeId " + homeId + " not an Accommodation in the
				...
				if (householdSublocationMap.containsKey(householdId)) {
					...
					if (homeSubLocationIndex + 1 > this.model.getLocationMap().get(homeId).getNumberOfSubLocations()) {
						System.err.println("Person " + personId + ". The homeId " + homeId + " with householdId "
								+ householdId + " has more sublocations (" + (homeSubLocationIndex + 1)
								+ ") than defined. Record" + " on row " + row.getOriginalLineNumber() + "\n"
								+ row.toString());
					}
				...
				case 2:
					if (!this.model.getLocationMap().get(workSchoolId).getLocationType().getName().toLowerCase()
							.equals("kindergarten")) {
						System.err.println(
								"workSchoolId " + workSchoolId + " not a Kindergarten in the location map on line "
										+ row.getOriginalLineNumber() + "\n" + row.toString());
						continue;
					}
				...
				case 4:
					...
					person = new SecondarySchoolStudent(this.model, personId, genderFemale, age, homeId,
							(short) this.model.getWeekPatternMap().get("0_Susceptible_secondary school student")
									.getId(),
							workSchoolId);
					break;
				...
				case 5:
					if (!this.model.getLocationMap().get(workSchoolId).getLocationType().getName().toLowerCase()
							.equals("college")) {
						System.err
								.println("workSchoolId " + workSchoolId + " not a College in the location map on line "
										+ row.getOriginalLineNumber() + "\n" + row.toString());
						continue;
					}
				...
				case 6:
					if (!this.model.getLocationMap().get(workSchoolId).getLocationType().getName().toLowerCase()
							.equals("university")) {
						System.err.println(
								"workSchoolId " + workSchoolId + " not a University in the location map on line "
										+ row.getOriginalLineNumber() + "\n" + row.toString());
						continue;
					}
				...
				case 12:
					...
					person = new WorkerSatelliteToCity(this.model, personId, genderFemale, age, homeId,
							(short) this.model.getWeekPatternMap().get("0_Susceptible_worker satellite to city")
									.getId(),
							workSchoolId);
					break;
				...
				case 13:
					...
					person = new WorkerCityToSatellite(this.model, personId, genderFemale, age, homeId,
							(short) this.model.getWeekPatternMap().get("0_Susceptible_worker city to satellite")
									.getId(),
							workSchoolId);
					break;
				...
				case 14:
					if (workSchoolId == -1 || !this.model.getLocationMap().containsKey(workSchoolId)) {
						System.err.println(
								"No work location [" + workSchoolId + "] for WorkerSatelliteToSatellite on line "
										+ row.getOriginalLineNumber() + "\n" + row.toString());
						continue;
					}
					// note: work location can be anything: school, retail, office, park, ...
	...
	private void readWeekpatternData() throws Exception {
			...
			if (row.getRowNum() > 0) {
				...
				if (distributionString != "") {
					...
					case "uniform":
						if (min > max)
							throw new MedlabsException(
									"uniform distribution with min > max in activity xlsx file at row "
											+ (row.getRowNum() + 1));
		...
		// {
		// Map<String, DayPattern> dayActivitiesMap =
		// this.weekDayPattern.get(weekPatternKey);
		...
		// dayPatterns[4] = dayActivitiesMap.get("Friday") == null ? dayPatterns[0] :
		// dayActivitiesMap.get("Friday");
		// dayPatterns[5] = dayActivitiesMap.get("Saturday") == null ? dayPatterns[0] :
		// dayActivitiesMap.get("Saturday");
		// dayPatterns[6] = dayActivitiesMap.get("Sunday") == null ? dayPatterns[0] :
		// dayActivitiesMap.get("Sunday");
	...
	private LocatorInterface makeLocator(final String locatorString, final String locationType,
			final double maxDistance) throws MedlabsException {
		switch (locatorString) {
		case "HomeLocator":
			return new HomeLocator();
		case "CurrentLocator":
		...
		case "RandomLocatorChoice":
			if (locationType.contains(":")) {
				return new RandomLocatorChoice(new CurrentLocator(), resolveLocationTypeChoice(locationType),
						maxDistance, false);
			}
	...
	private SortedMap<Double, LocationType> resolveLocationTypeChoice(final String types) throws MedlabsException {
		...
		for (String entry : entries) {
			...
			if (prob == 0.0)
				continue;
			Throw.when(prob < 0.0, MedlabsException.class,
					"probability <= 0 in resolveLocationTypeChoice for types " + types);
	...
	private void makeFamilies() {
		...
		for (int homeLocationId : families.keys()) {
			...
			if (family.size() == 0)
				...
				System.out.println(
						"Home Location " + homeLocation.getId() + ": family size " + family.size() + ", sub = " + sub);
			else if (family.size() / sub > 7)
				System.out
						.println("Home Location " + homeLocation.getId() + ": family size > 7: " + family.size() / sub);
